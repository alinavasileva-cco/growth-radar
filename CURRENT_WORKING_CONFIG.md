...
**Версия:** 10.08.2026  
**Последний запуск:** `N5K-20260810-176`  
**Статус:** `PENDING_MAIN_RUN_LOG_APPEND` — данные интегрированы, integrity PASS; общий campaign run_log требует безопасного append RUN 175 и RUN 176  
...
- baseline_count: **0**;
- текущий канонический итог: **171**;
- текущий contactable итог: **171**;
- осталось до цели: **4829**;
...
- последний отчёт: `reports/run_2026-08-10_176.md`.

## Последний RUN

- найдено сырых кандидатов: **30**;
- квалифицировано: **1**;
- добавленная компания: `N5K-0171 СТО КОДОВ / ООО «СТО КОДОВ»`;
...
- orphan contacts: **0**;
- orphan evidence: **0**;
- blocker: campaign-level `data/campaigns/new_5000/run_log.csv` заканчивается на RUN 174; безопасный full-file append требуется для RUN 175 и RUN 176. Частичная перезапись не выполнялась;
